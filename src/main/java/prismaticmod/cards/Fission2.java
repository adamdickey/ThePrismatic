...
public class Fission2 extends BaseCard {
    public static final String ID = makeID("Fission"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new FissionAction(this.upgraded));
    }
    public void upgrade() {
        if (!this.upgraded) {
            upgradeName();

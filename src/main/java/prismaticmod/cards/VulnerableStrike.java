...
public class VulnerableStrike extends BaseCard {
    public static final String ID = makeID("Vulnerable Strike"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    private static final int vulnNumber = 1;
    private static final int UPG_Vuln = 1;

    public VulnerableStrike() {
        super(ID, info); //Pass the required information to the BaseCard constructor.
        setDamage(DAMAGE, UPG_DAMAGE); //Sets the card's damage and how much it changes when upgraded.
        setMagic(vulnNumber, UPG_Vuln);

...
public class OneForAll extends BaseCard {
    public static final String ID = makeID("One for All"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    private void zeroCostsToHand(AbstractPlayer p){
        if (!p.drawPile.isEmpty()){
            for (AbstractCard card : p.drawPile.group) {
                if (card.cost == 0 || card.freeToPlayOnce){
                    addToBot(new DrawToHandAction(card));
                }
            }
        }
    }

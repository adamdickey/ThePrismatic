...
public class Establishment2 extends BaseCard {
    public static final String ID = makeID("Establishment"); //makeID adds the mod ID, so the final ID will be something like "modID:MyCard"
    ...
    public void use(AbstractPlayer p, AbstractMonster m) {
        addToBot(new ApplyPowerAction(p, p, new EstablishmentPower(p, 1), 1));
    }
    public void upgrade() {
        if (!this.upgraded) {
            this.isInnate = true;
